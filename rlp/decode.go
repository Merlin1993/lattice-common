...
func makeDecoder(typ reflect.Type, tags tags) (dec decoder, err error) {
	...
	default:
		return nil, fmt.Errorf("rlp: types %v is not RLP-serializable", typ)
	}
}
...
// as nil. Non-empty values are decoded into a value of the element types,
...
func decodeInterface(s *Stream, val reflect.Value) error {
	if val.Type().NumMethod() != 0 {
		return fmt.Errorf("rlp: types %v is not RLP-serializable", val.Type())
	}
	kind, _, err := s.Kind()
	if err != nil {
		return err
...
func decodeDecoder(s *Stream, val reflect.Value) error {
	// Decoder instances are not handled using the pointer rule if the types
...
// types depend on the input structure. Stream does not keep an
// internal buffer. After decoding a value, the input reader will be
// positioned just before the types information for the next value.
//
...
type Stream struct {
	...
	byteval byte   // value of single byte in types tag
...
// Raw reads a raw encoded value including RLP types information.
...
func (s *Stream) Decode(val interface{}) error {
	...
	if decErr, ok := err.(*decodeError); ok && len(decErr.ctx) > 0 {
		// add decode target types to error so context has more meaning
