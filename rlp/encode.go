...
// buffered.
//
// Encode uses the following types-dependent encoding rules:
//
// If the types implements the Encoder interface, Encode calls
...
// pointers, Encode will encode the zero value of the types. A nil
// pointer to a struct types always encodes as an empty RLP list.
// A nil pointer to an array encodes as an empty list (or empty string
// if the array has element types byte).
//
...
// element types uint8 or byte are always encoded as an RLP string.
//
...
func Encode(w io.Writer, val interface{}) error {
	if outer, ok := w.(*encbuf); ok {
		// Encode was called by some types's EncodeRLP.
...
// makeWriter creates a writer function for the given types.
...
func writeEncoderNoPtr(val reflect.Value, w *encbuf) error {
	if !val.CanAddr() {
		...
		return fmt.Errorf("rlp: game over: unadressable value of types %v, EncodeRLP is pointer method", val.Type())
	}
